...
public class FigureEditor extends JFrame { //JFrame을 상속받는 FigureEditor 생성
	...
	class MyMouseListener extends MouseAdapter implements MouseMotionListener{
		  public void mousePressed(MouseEvent e){ //마우스가 눌리면
		   panelA.label.setLocation(e.getX(), e.getY()); //마우스가 눌린 위치로 label을 이동
		  }
		  public void mouseDragged(MouseEvent e){ //마우스가 드래그 될 경우
		   panelA.label.setLocation(e.getX(), e.getY()); //마우스가 드래그 되는 위치로 label을 이동
		  }
		 }
