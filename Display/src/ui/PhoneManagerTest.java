...
public class PhoneManagerTest {
	public static void main(String Args[]) {
		...
		while(true) { //무한반복
			...
			if (name.equals("exit")) //입력 받은 값이 "exit"이라면
				break; //반복문 빠져나오기
			ArrayList<Phone> result = manager.searchByName(name); //변수 manger에서 찾은 값을 배열 result에 넣기
			if (result != null) { //result에 입력 받은 값과 같은 값을 갖은 인덱스가 있다면
				for(int i=0; i<result.size(); i++) //처음부터 차례대로 result 인데스 확인
					System.out.printf("%s의 번호는 %s입니다.\n",name, result.toString()); //저장된 이름과 전화번호를 출력
			}else //만약 입력 받은 값과 같은 값을 갖은 인덱스가 없다면
